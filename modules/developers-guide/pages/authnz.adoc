...
----
 ...
 docker-compose up -d keycloak //<4>

 # Bring up the stargate
 docker-compose up -d stargate-jwt //<5>
 # Wait until stargate is up before bringing up the metrics tools
 echo ""
 echo "Waiting for stargate to start up..." //<6>
 ...
 docker run --rm -it -v /Users/lorina.poland/CLONES/stargate/docker-images/examples/stargate-jwt-auth/scripts:/scripts  \ //<3>
...
<3> Use `docker run nuvo/docker-cqlsh` to insert some data into Cassandra
and create a role and grant permissions.
<4> Start keycloak before stargate.
<5> Start stargate node.
<6> Some logic to monitor stargate progress.
