...
TIP: If you are using Stargate as part of DataStax's Astra DB cloud-native database, you can define roles and permissions using the https://astra.datastax.com/[Astra console].
...
version: '2'
  ...
  services:
    ...
    stargate-jwt:
      ...
      mem_limit: 2G
      environment: //<6>
        - JAVA_OPTS=-XX:+CrashOnOutOfMemoryError -Xmx750M -Xms64M -Dstargate.auth_id=AuthJwtService -Dstargate.cql_use_auth_service=true -Dstargate.auth.jwt_provider_url=https://keycloak:4444/auth/realms/stargate/protocol/openid-connect/certs
