...
by the `Book` object type. The output is a Boolean value. The `@cql_update` directive
makes this mutation an update, rather than an insert.
...
// LLP: is this true?
// turning a "legacy" cql keyspace into a graphql schema-first keyspace
