...
| @cql_increment | Set a type field that will increment on UPDATE | field to increment (counter, set, and list only); The default is false, meaning that the value will be appended; prepend: Boolean = false
| @cql_if | Set a type field that the predicate `if <field> <predicate` uses to check condition for  DELETE and UPDATE. | field, predicates (EQ, GT, GTE, IN, GT, LTE, and NEQ)
| @cql_index | Create a type field index | name, class (org.apache.cassandra.index.sasi.SASIIndex), type (FULL, VALUES), options (mode: 'CONTAINS')
...
| @cql_where | Predicates that customize the conditions of a parameter | field, predicate (EQ (default), GT, GTE, IN, LT, LTE, CONTAINS)
|===
