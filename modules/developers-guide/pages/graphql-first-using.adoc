= Using the Stargate GraphQL API (graphql-first)
...
Read xref:graphql-first-using.adoc#deploying_schema[Deploying Schema].
====
...
added to generate an `AddressInput` type with the same fields as the type `address`
