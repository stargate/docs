...
=== Using the Stargate docker image cqlsh
...
Alternatively, you can install a standalone CQL shell using a docker image.
Get the IP address of your Stargate docker container (assuming the Stargate
 container name is "stargate"):

[source,bash]
----
docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' stargate
docker pull nuvo/docker-cqlsh {ip-address} --cqlversion=3.4.5
----

Connect to your Stargate container using `cqlsh`:
