...
be added to the response from insertions.

or annotating it with @cql_insert(ifNotExists: true)
// end::CreateMutations[]
...
They only serve as wrappers of an operation's arguments or response.
...
An example of retrieving data using this query is found xref:graphql-oriented-using.adoc#_retrieving_data[below].
...
will be true and user will echo back the data that was just inserted; otherwise, applied will be false and user will be the existing data.
...
| GT (greater than) | clustering key, regular indexed field
| GTE (greater than or equal to) | clustering key, regular indexed field
| LT (less than) | clustering key, regular indexed field
| LTE (less than or equal to) | clustering key, regular indexed field
...
name with `update`.
A mutation will also be an update operation if it is annotated with `@cxql_update`.
...
After adding the book "Pride and Prejudice" with an `insertBook()`, you can delete
...
// LLP: Is this true? Note the use of `ifExists` to validate that the book exists before deleting it.
// end::DeleteData[]
...
| @cql_index | Create a type field index | name, class, type, options
...
| @cql_update | |
| @cql_where | Predicates that customize the conditions of a parameter | EQ (default), IN, GT, GTE, LT, LTE, CONTAINS
|===
// end::Directives[]

// LLP: Apollo data federation - in this doc, or a separate page?
