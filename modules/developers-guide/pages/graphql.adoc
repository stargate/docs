...
one for deploying schema in the GraphQL-oriented, and the third for querying a keyspace.
The URLS are:
...
The schema endpoint is used to create or alter CQL schema in the CQL-oriented GraphQL using direct schema manipulation.
The admin endpoint is used to deploy GraphQL schema in the GraphQL-oriented GraphQL which will indirectly modify underlying CQL schema.
