...
Finally, if no `WHERE` clause or primary key is provided and `/rows` is added 
to the path then all rows in the table are returned. 

CAUTION: Returning *all* rows in a large table can negatively impact your database.
For large tables it is recommended to include a `page-size` query parameter in your
request to limit the number of results returned and to further page through the remaining
rows by also including the `page-state` parameter in subsequent requests.
