...
// LLP: 05.24.21 ADD CREATE UDT HERE
// LLP: 05.24.21 ADD DELETE UDT HERE
...
// LLP: 05.24.21 ADD CREATE INDEX AND DELETE INDEX?
...
include::developers-guide:page$create-an-index-rest.adoc[]

include::developers-guide:page$delete-an-index-rest.adoc[]
...
[NOTE]
====
Notice that in the below example both `Mookie` and `Betts` are supplied in the
path. In this example, `Mookie` being the `firstname` is the partition key and
`Betts` being the `lastname` is the clustering key which together form the primary
key of the row. If there were more partition keys or clustering keys on the table
each could be provided as `pk1/pk2/ck1/ck2/...` each separated with a forward
slash. Although some of the clustering keys can be omitted in order to return
additional rows the preceding keys must be provided. This means that if there is
one partition key `pk1` and three clustering keys `ck1`, `ck2`, and `ck3` then `pk1`,
`ck1` and `ck2` are required to return all rows with the clustering key of `ck2`.
====
