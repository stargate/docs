...
With a collection of many documents it can be helpful to page through only
a subset of the results at a time. For this example, we will use a `page-size`
of 5. Start by sending a `GET` request to
`/v2/namespaces/{namespace_name}/collections/{collections_name}page-size=5` to
retrieve first page of documents in the collection:
...
To get the next 5 documents re-run the same request, but add the `page-state`
parameter along with the value returned from the previous results:
