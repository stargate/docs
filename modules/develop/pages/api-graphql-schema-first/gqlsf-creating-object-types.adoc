...
The `@cql_index` is discussed in xref:graphql-first-using.adoc#create-indexes[Create indexes].
Not required.
...
`reviews: [Review]`.
Note that `reviews` and `addresses` are arrays of xref:graphql-first-using.adoc#create-a-user-defined-type-udt[UDTs]
