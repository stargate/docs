...
. xref:manage:org/managing-org.adoc#_manage_application_tokens[Manage application token].
...
include::develop/api-rest/rest-checking-keyspace.adoc[]
...
Send a `POST` request to `{base_rest_schema}/{rkeyspace}/tables` to create a table.
...
Notice that each column must have a xref:dev-with-rest-api.adoc#data-types[data type] specified.
