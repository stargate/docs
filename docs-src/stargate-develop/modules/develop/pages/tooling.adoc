...
The tool includes a https://learning.postman.com/docs/running-collections/intro-to-collection-runs/[collection runner testing utility,window="_blank"]
for testing against query assertions embedded in the collection.
Collections and environments can be https://learning.postman.com/docs/getting-started/importing-and-exporting-data/[imported or exported,window="_blank"]
with Postman.
...
* Install http://www.postman.com[Postman,window="_blank"] to run postman testing.
...
Two environments are also available, one for the open-source Stargate docker image and one for https://astra.datastax.com[DataStax Astra,window="_blank"].
...
https://learning.postman.com/docs/sending-requests/managing-environments/[change them in the Postman environment,window="_blank"].
You can also edit the JSON file directly, if you prefer.

==== Astra

There are three items that must be
https://docs.datastax.com/en/astra/docs/manage-application-tokens.html[retrieved from the Astra dashboard,window="_blank"] and configured in the Postman environment:

* ASTRA_DB_ID
* ASTRA_DB_REGION
* ASTRA_DB_APPLICATION_TOKEN

Open the https://learning.postman.com/docs/sending-requests/managing-environments/[environment in Postman,window="_blank"]
...
In Astra, go to the Connect tab for your database, choose GraphQL under the
`Connect using an API` and you'll see instructions for accessing the playground.
In a browser, you can connect at:
`\https://$ASTRA_CLUSTER_ID-$ASTRA_REGION.apps.astra.datastax.com/api/rest/swagger-ui/`
after substituting your values for `$ASTRA_CLUSTER_ID` and `$ASTRA_REGION`.
endif::[]
ifeval::["{database}" == "Stargate"]
...
* Install https://github.com/bloomrpc/bloomrpc#installation[BloomRPC,window="_blank"] to run gRPC testing
...
https://github.com/stargate/stargate/tree/master/grpc-proto/proto[imported into BloomRPC,window="_blank"].
