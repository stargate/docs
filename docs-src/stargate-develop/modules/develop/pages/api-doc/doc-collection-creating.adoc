...
In the Document API, collections are created in a {glossary-url}gloss_namespace.html[namespace]. 
...
In this example we use `\{collection_name}` for the `name`.
...
To set JSON schema that a collection's documents will use, send a `PUT` request to `{base_doc_api}/\{namespace_name}/collections/\{collection_name}`.
