...
curl-document-get-6.sh

=== Paging-state

curl-document-get-all-docs-with-paging.sh

=== Fields

curl-document-get-one-with-fields.sh

== Search collection for documents with a simple WHERE clause

Get all documents where a Name equals a particular reader

curl-document-get-where-name.sh

== Search collection for documents with a simple WHERE clause with fields

Get all documents where name eq a particular reader with fields.

curl-document-get-where-name-eq.sh

== Search collections for documents with operators: eq, ne, or, not, gt, gte, lt, lte, in, nin
