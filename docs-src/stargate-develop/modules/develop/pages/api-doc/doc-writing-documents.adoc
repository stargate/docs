...
Send a `POST` request to `{base_doc_api}/{namespace_name}/collections/{collection_name}` similar to the last example, but add more JSON data to the body of the request:
