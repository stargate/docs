...
Send a `PUT` request to `{base_doc_api}/\{namespace_name}/collections/\{collection_name}/\{document-id}` to add data to the collection `\{collection_name}`.
...
Send a `POST` request to `{base_doc_api}/\{namespace_name}/collections/\{collection_name}`
...
Send a `POST` request to `{base_doc_api}/{\namespace_name}/collections/\{collection_name}` similar to the last example, but add more JSON data to the body of the request:
