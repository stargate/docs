...
https://cassandra.apache.org/doc/latest/configuration/cass_yaml_file.html#authenticator[cassandra.yaml,window="_blank"] file.
If you don't have a cluster running, use these instructions to install
https://cassandra.apache.org/doc/latest/getting_started/installing.html#choosing-an-installation-method[Cassandra 3.11,window="_blank"].

Next, prepare a virtual machine to install Stargate on. You'll need to be sure
Java 8 is installed.

Once you have a machine ready,
download the https://github.com/stargate/stargate/releases/download/{stargate-docker-tag-3x}/stargate-jars.zip[Stargate zip file,window="_blank"].
This file will provide the jar files that are required to run Stargate.
A typical method is using wget:

[source, shell, subs="attributes+"]
----
include::example$bash/wget_sg_jars_cass_3x.sh[]
----
...
[source, shell, subs="attributes+"]
----
include::example$bash/start_vm_3x.sh[]
----
